...
В нашем списке FeedList сами сами ячейки таблицы слишком маленькие по высоте[ПОКАЗЫВАЕМ ПАЛЬЦАМИ], чтобы впихнуть туда еще и картинку и скажем дату. Зайдем в файл Main.storyboard выберем наш FeedViewController, в нем выберем feedList и в Size Inspector найдем параметр Row Height, снимем галочку automatic и введем 256.
...
2. Далее изменим внешний вид нашей ячейки. Добавим пару ImageView. Первый будет подгружать картинку по ссылке, а второй будет статичным  для улучшения внешнего вида. Так же добавим Label. В нем будет такой текст (☝️). 
